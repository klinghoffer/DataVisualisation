class Thing {
  ...
  Thing(int startX, int startY, float mass, float radius, float gravitationalRange, String name) {
    position = new PVector(startX, startY);
    velocity = new PVector(0, 0);
    acceleration = new PVector(0, 0);
    this.radius = radius;
    this.mass = mass;
    this.gravitationalRange = gravitationalRange;
    this.name = name;
  }
  ...
  void draw() {
    ...
    fill(0);
    text(name, position.x + radius * 2, position.y + radius * 2);
